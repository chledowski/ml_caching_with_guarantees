...
def evaluate(policy_model, data, step, descriptor, tb_writer, log_dir, k=5):
    ...
    for batch in tqdm.tqdm(zip(*subsequences), desc=desc, total=subseq_length):
        ...
        for i in range(FLAGS.batch_size):
            logs[i].append(pretty_print(  # TODO: make this more efficient than pretty print..
...
def main(_):
    ...
    if FLAGS.evaluate:
        print(fr"Picking the best checkpoint by checking {FLAGS.experiment_base_dir}logs.txt")
        with open(fr'{FLAGS.experiment_base_dir}logs.txt', "w") as f:
